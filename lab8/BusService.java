...
class BusService {
  ...
  public CompletableFuture<Set<BusStop>> getBusStops() {
    return BusAPI.getBusStopsServedBy(serviceId).thenApply(x -> (new Scanner(x)).useDelimiter("\n").tokens()
        .map(line -> line.split(",")).map(fields -> new BusStop(fields[0], fields[1])).collect(Collectors.toSet()));
  }
  ...
  public CompletableFuture<Set<BusStop>> findStopsWith(String name) {
    return getBusStops().thenApply(x -> x.stream().filter(stop -> stop.matchName(name)).collect(Collectors.toSet()));
  }
