...
/**
 * The Disease Outbreak Response Management System (DORMS) is a system for
 * tracking and managing disease outbreaks in Singapore.
 */
public class Dorms {

    /**
     * Currently, DORMS is managing the SARS-CoV-2 outbreak that causes COVID-19.
     */
    ...
    private static final String TESTS_POSITIVE = "tests positive for %s " + "at time";
    private static final String TESTS_NEGATIVE = "test negative for %s " + "at time";
     ...
     * Initialises DORMS
     * 
     * @param verbose Initialises DORMS in verbose mode, i.e. log messages will be
     ...
     */
    public final void checkIn(String personName, boolean isWearingMask, String locationName, double time) {
     ...
     */
    public final void checkOut(String personName, String locationName, double time) {
        // Get the most updated state of the Person
     ...
     */
    public final void contact(String personA, boolean isAWearingMask, String personB, boolean isBWearingMask,
            double time) {
    ...
    /**
     * Indicates to DORMS that someone has presented with symptoms of the target
     * virus
     * 
     * @param personName The person's name
     * @param time       The time this happened
     */
    public final void presentSymptoms(String personName, double time) {
        Person person = personDatabase.getPerson(personName);
    ...
    /**
     * This transmits viruses between two people sequentially, updating the state of
     * each Person at each iteration using {@link PersonDatabase#update}
     * 
    ...
    private final void handleNewContacts(List<Contact> listOfContacts) {
        ...
        for (Contact c : listOfContacts) {
            ...
            List<Person> personList = d.transmit(RandomNumberGenerator.nextDouble());
            personDatabase.update(personList);
            rebuild.add(d);
        }
     ...
     * @return The contacts that involved this Person that happened after
     *         {@code time -} {@link SimulationParameters#TRACING_PERIOD}
     */
    final List<? extends Contact> queryContacts(String personName, double time) {
        ...
        for (Contact c : listOfContacts) {
            ...
            boolean involvesPerson = firstName.equals(personName) || secondName.equals(personName);
            boolean isWithinTime = time - SimulationParameters.TRACING_PERIOD < timeOfContact;
    ...
    private final void logNegativeTest(Person person, double time) {
        log(String.format(buildLog(TESTS_NEGATIVE), person, SimulationParameters.TARGET_VIRUS, time));
    }

    private final void logPositiveTest(Person person, double time) {
        log(String.format(buildLog(TESTS_POSITIVE), person, SimulationParameters.TARGET_VIRUS, time));
    }
