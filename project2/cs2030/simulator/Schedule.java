...
public class Schedule {
     ...
     */
    public Schedule(int numOfServers, List<Double> arrivalTime, int queueCapacity) {
        ...
        for (int i = 0; i < numOfServers; i++) {
            serversList.add(new Server(i + 1, true, false, 0, queueCapacity));
        }
    ...
    private List<Number> setStatistics() {
        ...
        for (Event newEvent : this.eventsList) {
            ...
            if (newEvent instanceof WaitEvent) {
                double nextAvailableTime = newEvent.getCurrentServer().getNextAvailableTime();
       ...
       RandomGenerator random = new RandomGenerator();
        ...
        while (!tempEventQueue.isEmpty()) {
            ...
            if (newEvent.getCurrentServer() != null) {
                currenServerList.set(newEvent.getCurrentServer().getIdentifier() - 1, newEvent.getCurrentServer());
            }
            ...
            if (newEvent instanceof ArriveEvent) {
                ...
                nextEvent = arriveEvent.execute(new Shop(currenServerList)).second();
            }
